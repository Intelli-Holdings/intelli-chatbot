...
export default function CreateTemplateForm({
  ...
  const [templateData, setTemplateData] = useState({
    ...
    carouselData: null as any,
    flowId: "",
    flowAction: "navigate" as "navigate" | "data_exchange",
    flowScreen: "",
  });
  ...
  const categoryConfig = {
    MARKETING: {
      subcategories: ["custom", "carousel", "catalogue", "flows"],
      label: "Marketing",
    },
    UTILITY: {
      subcategories: ["custom", "flows"],
      label: "Utility",
    },
    AUTHENTICATION: {
      subcategories: ["otp"],
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!limits.formats.includes(fileExtension)) {
      toast.error(
        `Invalid file format. Accepted formats: ${limits.formats.join(", ")}`
      );
      return;
    }
  ...
  const addButton = () => {
    ...
    const urlButtonCount = templateData.buttons.filter(
      (b) => b.type === "URL"
    ).length;
    const phoneButtonCount = templateData.buttons.filter(
    ...
    try {
      ...
      if (
        ...
        try {
          ...
          headerMediaHandle = await uploadMediaToMeta(
            templateData.headerMediaFile,
            appService
          );
      ...
      const formattedTemplate = TemplateCreationHandler.createTemplate(
        templateDataWithMedia
      );
                ...
                {templateData.buttons.length > 0 && (
                  <div className="flex flex-col gap-1 pt-2 border-t">
                    {templateData.buttons.map((button, index) => (
                      <div
                        key={index}
                        className="text-center text-sm text-blue-600 font-medium py-1"
                      >
        ...
        <div className="max-w-2xl mx-auto space-y-6">
          <div className="text-center">
            ...
            <p className="text-muted-foreground">
              Choose category and provide basic information
            </p>
          </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              {Object.entries(categoryConfig).map(([key, config]) => (
                <Button
                  ...
                  type="button"
                  variant={
                    templateData.category === key ? "default" : "outline"
                  }
          ...
          <div className="space-y-3">
            ...
            <RadioGroup
              ...
              onValueChange={(value) =>
                setTemplateData({ ...templateData, subcategory: value as any })
              }
            >
              <div className="space-y-2">
                {categoryConfig[templateData.category].subcategories.map(
                  (sub) => (
                    <div
                      key={sub}
      ...
      {step === 2 && (
        <>
          ...
          {templateData.subcategory === "carousel" ? (
            ...
            />
          ) : templateData.subcategory === "flows" ? (
            <FlowsTemplateWrapper
              templateName={templateData.name}
              language={templateData.language}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_400px] gap-6">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    {templateData.name} •{" "}
                    {categoryConfig[templateData.category].label}
                  </p>
                </div>
                ...
                <div className="space-y-3">
                      ...
                      <Button
                        ...
                        onClick={() =>
                          setTemplateData({
                            ...
                            headerText:
                              type === "TEXT" ? templateData.headerText : "",
                            headerMediaFile: [
                              "IMAGE",
                              "VIDEO",
                              "DOCUMENT",
                      ...
                      {templateData.headerVariables.length > 0 && (
                        <Alert>
                          ...
                          <AlertDescription className="text-xs">
                            Header can only have 1 variable. Variable:{" "}
                            {templateData.headerVariables[0]}
                          </AlertDescription>
                        </Alert>
                      )}
                    </div>
                  ...
                  {["IMAGE", "VIDEO", "DOCUMENT"].includes(
                    ...
                    <div className="space-y-2">
                      <Alert>
                        ...
                        <AlertDescription className="text-xs">
                          Upload a sample{" "}
                          {templateData.headerType.toLowerCase()} file for
                          template approval
                        </AlertDescription>
                      </Alert>
                      ...
                      <input
                        ...
                        accept={
                          MEDIA_LIMITS[
                            templateData.headerType as keyof typeof MEDIA_LIMITS
                          ]?.accept
                        }
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              ...
                              <div>
                                ...
                                <p className="text-xs text-muted-foreground">
                                  {(
                                    templateData.headerMediaFile.size / 1024
                                  ).toFixed(2)}{" "}
                                  KB
                                </p>
                              </div>
                  ...
                  {templateData.headerType === "LOCATION" && (
                    <Alert>
                      ...
                      <AlertDescription className="text-xs">
                        Location will be specified when sending the message. The
                        template will show a map placeholder.
                      </AlertDescription>
                    </Alert>
                  )}
                </div>
                                  ...
                                  {button.type === "FLOW" && (
                                    <div className="space-y-2">
                                      <Alert>
                                        <Info className="h-4 w-4" />
                                        <AlertDescription className="text-xs">
                                          Create and publish your Flow in
                                          WhatsApp Manager first, then enter the
                                          Flow ID and screen ID here.
                                        </AlertDescription>
                                      </Alert>
              ...
              {/* Right Panel: Preview */}
              <div className="lg:block hidden">{renderPreview()}</div>
            </div>
          )}
        </>
      )}
