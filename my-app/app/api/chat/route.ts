...
import { chatMessageSchema, validateBody } from '@/lib/validations/api';
import { chatLimiter } from '@/lib/rate-limit';
import { sanitizeUserPrompt, scrubPiiFromOutput, validateInputLength } from '@/lib/ai-safety';
...
export async function POST(request: Request) {
  try {
    ...
    const rateLimitResult = chatLimiter(ip);
    ...
    logger.debug('Raw request body', { data: rawBody });

    // Parse the body
    const body = JSON.parse(rawBody);
    logger.debug('Parsed request body', { data: body });
    ...
    const lengthError = validateInputLength(userMessage);
    if (lengthError) {
    ...
    // Scrub PII from AI response before returning
    const scrubbedResponse = scrubPiiFromOutput(responseMessage);
    return NextResponse.json({ response: scrubbedResponse });
