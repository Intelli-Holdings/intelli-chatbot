...
export async function POST(
  ...
  try {
    ...
    const url = `${BASE_URL}/appservice/${appserviceId}/coexistence/sync/`
    ...
    if (!contentType.includes("application/json")) {
      ...
      logger.error("Coexistence sync: non-JSON response", { status: response.status, body: text.slice(0, 500) })
    ...
    logger.error("Error initiating coexistence sync", { error: message })
